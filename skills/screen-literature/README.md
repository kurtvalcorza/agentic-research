...
Canonical pipeline order: `design-review-protocol` → `generate-screening-criteria` → `acquire-corpus` → `dedupe-records` → **screen-literature** (single-pass or DUAL) → extract / synthesize → `appraise-risk-of-bias` → `validate-evidence` / draft → `prisma-flow`.
