...
- Canonical order: `design-review-protocol` → `generate-screening-criteria` → `acquire-corpus` → `dedupe-records` → **`screen-literature`** (single-pass or DUAL) → extract/synthesize → `appraise-risk-of-bias` → `validate-evidence` (GRADE) / draft → `validate-*` + `verify-sources` → `prisma-flow` → `verify-review` (loop to verified end-state; consumes the `prisma-flow` reconciliation as `U_prisma`).
