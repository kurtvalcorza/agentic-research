...
def render(table: tuple, entries: dict, errs: list[str], variant: str,
    ...
    lines += ["",
              ...
              "---",
              "",
              # The source record belongs in the provenance line, as it does in the
              # GRADE and appraisal generators: once this artifact is saved or moved,
              # two checklists generated from different records were indistinguishable
              # in the audit trail, which is what the provenance line exists to prevent.
              f"*Generated by `prisma_checklist.py` from `{source}` (variant `{variant}`). "
              f"This check verifies that a location or justification was recorded for every "
              f"row. It cannot verify that the cited location actually addresses the item.*"]
    return "\n".join(lines)
