...
- **No-acquisition note**: If Phase 0 was skipped (bring-your-own-corpus), there is no documented database search to report, but the corpus itself is a real identification source and must be declared as one — a record with no identification count is rejected outright (exit 2), because a flow naming neither end reconciles trivially and certifies nothing. Write `"identified_databases": {"pre-collected corpus": N}` where N is the corpus size, `"duplicates_removed": 0` if no de-duplication was performed (that is a fact about the run, not a placeholder), and use N as the records-screened starting point. Note in `prisma-flow.md` that identification was a pre-collected corpus, not a documented database search — the label carries that meaning, and the left-hand arm is where the corpus belongs structurally because it is the only arm with a title/abstract screening stage. Do **not** invent identification or duplicate numbers; `prisma-flow` consumes real counts only, and the corpus size is a real one.
