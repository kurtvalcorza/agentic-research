...
1. **Setup**: Run the platform-native prerequisite check from repo root: on Unix/Linux/macOS, `bash .specify/scripts/bash/check-prerequisites.sh --json`; on Windows, `pwsh .specify/scripts/powershell/check-prerequisites.ps1 -Json`. Parse JSON for FEATURE_DIR and AVAILABLE_DOCS list.
