...
class TestGolden(unittest.TestCase):
    def test_generated_checklist_matches_golden(self):
        """Everything above the provenance line, which embeds the invocation's path.

        The provenance line now names the source record — so it differs by machine,
        exactly as grade_profile.py's does, and the golden covers the artifact above
        it. The line itself is pinned by the test below, which is the part that
        matters: that it names the record at all.
        """
        _, out, _ = run(fixture("checklist.valid.json"))
    ...
    def test_provenance_names_the_source_record(self):
        """Two checklists generated from different records were indistinguishable
        once saved — which is what the provenance line exists to prevent."""
        _, out, _ = run(fixture("checklist.valid.json"))
        self.assertIn("Generated by `prisma_checklist.py` from `", out)
        self.assertIn("checklist.valid.json", out)
        self.assertIn("variant `prisma_2020`", out)

    def test_provenance_names_stdin_when_there_is_no_file(self):
        out, err = io.StringIO(), io.StringIO()
        record = fixture("checklist.valid.json").read_text(encoding="utf-8")
        with mock.patch.object(sys, "argv", ["prisma_checklist.py"]), \
                mock.patch.object(sys, "stdin", io.StringIO(record)), \
                redirect_stdout(out), redirect_stderr(err):
            pc.main()
        self.assertIn("from `stdin`", out.getvalue())
