...
- [x] T060 Open the pull request, calling out both `prisma_flow.py` behavioural changes — the coercion rule (T008, D-019) and the closed schema with a required `schema_version` (D-020) and any pre-existing defects surfaced by T044/T050 as separate items rather than folded into the feature, then run the dual-bot review loop to clean or false-positives-only before merge
